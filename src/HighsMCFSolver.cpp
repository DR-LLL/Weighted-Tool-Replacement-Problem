...
double HighsMCFSolver::ComputeSolution(const TestInstance& instance) {
    Highs highs;
    ...
    std::cout << "[ILP] status=" << highs.modelStatusToString(highs.getModelStatus())
              << ", primal_status=" << info.primal_solution_status
              << ", gap=" << info.mip_gap << '\n';
    const double obj = highs.getObjectiveValue();

    if (run_status == HighsStatus::kError || !highs.getSolution().value_valid ||
        info.primal_solution_status != kSolutionStatusFeasible || !std::isfinite(obj)) {
        throw std::runtime_error("HiGHS: no finite incumbent objective");
    }

    // For article experiments this ILP is a time-limited baseline: when HiGHS
    // stops before proving optimality, a finite incumbent is still meaningful.
