...
void DAG::init_potentials_by_topo_(std::vector<long long>& pi) const {
    ...
    for (int u : topo) {
        ...
        for (Arc* a : g_[u]) {
            ...
            if (fwd->residual_cap() <= 0) continue;
            int v = a->head();
            long long nd = dist[u] + a->cost();
...
std::pair<int, std::vector<std::vector<int>>> DAG::compute_max_flow_min_cost(int required_flow) {
    ...
    if (required_flow > 0 && total_flow != required_flow) {
        throw std::runtime_error("max-flow value is smaller than required flow");
    }
