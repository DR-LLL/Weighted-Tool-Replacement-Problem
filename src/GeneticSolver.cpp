...
namespace {
double retainedScoreWeight(int numJobs) {
    return std::min(0.5, 4.5 / static_cast<double>(numJobs));
}
}


double GeneticSolver::ComputeSolution(const TestInstance& instance) {
    ...
    double totalTime = 0;
    const unsigned int seed = static_cast<unsigned int>(std::rand() % 2147483646 + 1);

    // Конвертируем TestInstance в параметры, необходимые для генетического алгоритма
    parameters = createParametersFromInstance(instance, seed);
    ...
    const Individual* bestIndividual = population->getBestIndividual();
    const double evaluationCost = bestIndividual->solutionCost.evaluation;
    const double retainedCost = bestIndividual->solutionCost.costs;
    const double retainedWeight = retainedScoreWeight(instance.N);
    cost = (1.0 - retainedWeight) * evaluationCost + retainedWeight * retainedCost;
...
Parameters* GeneticSolver::createParametersFromInstance(const TestInstance& instance, unsigned int seed) {
