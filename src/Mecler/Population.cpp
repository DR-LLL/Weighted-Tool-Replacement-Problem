...
Population::Population(Parameters *parameters,double timer) : parameters(parameters) {
    ...
    for (unsigned int i = 0; i < parameters->populationSize; i++) {
        ...
        if (std::chrono::duration_cast<std::chrono::microseconds>(t1 - t0).count() / 1000000.0 > timer){
            parameters->populationSize = i+1;
            break;
        }
    }
}
